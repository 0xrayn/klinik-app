    ...
    return (
        <>
            ...
            <aside className={clsx(
                ...
                {/* Collapse toggle — floating on the edge, beside the header */}
                <button onClick={onToggleCollapse}
                    className="hidden lg:flex absolute -right-3 top-5 z-10 items-center justify-center w-6 h-6 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-400 shadow-sm hover:text-brand-600 hover:border-brand-200 dark:hover:border-brand-500/40 transition-colors">
                    {collapsed ? <ChevronRightIcon className="w-3.5 h-3.5" /> : <ChevronLeftIcon className="w-3.5 h-3.5" />}
                </button>

                {/* Logo */}
                <div className={clsx('flex items-center h-14 px-4 border-b border-slate-100 dark:border-slate-800 shrink-0', collapsed ? 'lg:justify-center lg:px-0 justify-between' : 'justify-between')}>
                    <div className="flex items-center gap-2.5 overflow-hidden">
                        <div className="w-8 h-8 bg-gradient-to-br from-brand-500 to-brand-700 rounded-xl flex items-center justify-center shadow-brand shrink-0">
                        ...
                        <div className={clsx('leading-tight whitespace-nowrap', collapsed && 'lg:hidden')}>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">Klinik Sehat</p>
